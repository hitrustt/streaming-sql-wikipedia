      ...
      <div className="panel__head">
        ...
        {result.meta && (
          <span className="panel__meta">
            ...
            {formatWindow(result.meta.windowSeconds)} · {result.meta.elapsedMs.toFixed(1)} ms
          </span>
        )}
      </div>
...
function BarList({ result, rows }: { result: LiveResult; rows: Array<{ key: string; row: Row }> }) {
  ...
  const valueIndex = result.columns.findIndex(
    (_, index) => index >= Math.max(1, result.meta?.keyColumns ?? 1) &&
      rows.some(({ row }) => typeof row[index] === 'number'),
  );
  ...
  const keyCount = Math.max(1, result.meta?.keyColumns ?? 1);
