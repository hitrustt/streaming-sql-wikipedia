...
export default function App() {
  const {
    ready, presets, columns, stats, ingest, events, headline, headlineColumns, result,
  ...
  // Start the default query as soon as the engine is up, so a visitor who never
  // interacts still sees live results moments after landing.
  useEffect(() => {
  ...
  return (
    <div className="app">
      ...
      {/* Two distinct states worth telling the user about: still catching up
          (normal, brief, and explains why numbers are small), and genuinely
          disconnected (abnormal, and self-healing). */}
      {ingest?.backfilling && (
        <div className="banner banner--info">
          <span>Loading the last few minutes of edits…</span>
        </div>
      )}

      {ingest && !ingest.connected && !ingest.backfilling && (
        <div className="banner">
          <span>Lost the connection to Wikipedia's stream — reconnecting automatically.</span>
        </div>
      )}
